...
void ofApp::draw(){
    ...
    if(toggleBetweenMesh == 0){
        myOscillatingObject.draw();
    } else if(toggleBetweenMesh == 1){
        myTwistingKnot.draw();
    }
    
    button.draw(5, 5, 150, 150);
    button1.draw(170, 5, 150, 150); 
}
...
void ofApp::touchDown(ofTouchEventArgs & touch){
    ...
    if(ofGetMouseX() >=5 && ofGetMouseX() <=155 && ofGetMouseY() >=5 && ofGetMouseY() <=155){
        cout << "buton is working" << endl;
        toggleBetweenMesh = 0;
    }
    
    if(ofGetMouseX() >=170 && ofGetMouseX() <=320 && ofGetMouseY() >=5 && ofGetMouseY() <=155){
        cout << "buton 2 is working" << endl;
        toggleBetweenMesh = 1;
    }
}
